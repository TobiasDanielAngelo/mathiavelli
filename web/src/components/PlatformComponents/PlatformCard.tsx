...
export const PlatformCard = observer(
  (props: { item: Platform; shownFields?: (keyof PlatformInterface)[] }) => {
    ...
    const onDelete = async () => {
      const resp = await platformStore.deleteItem(item?.id ?? -1);
      if (!resp.ok) {
        setMsg(resp.details);
        return;
      }
    ...
    return (
      <div className="m-1 border-gray-700 rounded-lg p-5 border">
        <MyModal isVisible={isVisible1} setVisible={setVisible1}>
          <PlatformForm item={item} setVisible={setVisible1} />
        </MyModal>
        <MyConfirmModal
