...
export const FollowUpCard = observer(
  (props: { item: FollowUp; shownFields?: (keyof FollowUpInterface)[] }) => {
    ...
    const onDelete = async () => {
      const resp = await followUpStore.deleteItem(item?.id ?? -1);
      if (!resp.ok) {
        setMsg(resp.details);
        return;
      }
    ...
    return (
      <div className="m-1 border-gray-700 rounded-lg p-5 border">
        <MyModal isVisible={isVisible1} setVisible={setVisible1}>
          <FollowUpForm item={item} setVisible={setVisible1} />
        </MyModal>
        <MyConfirmModal
